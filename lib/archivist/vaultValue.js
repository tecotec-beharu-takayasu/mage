const mage = require('mage/lib/mage');
	...
	for (var i = 0; i < keys.length; i++) {
		...
		if (type !== 'string' && type !== 'number') {
			throw new TypeError(
				'Index value for "' +
					key +
					'" is not a string or number. Found: ' +
					value
			);
		}
...
function VaultValue(topic, index, mediaType) {
	...
	// HACK
	console.log('[VaultValue] created', this.topic, this.index, this.mediaType);
...
VaultValue.prototype.applyDiff = function (diff) {
	...
	if (!api) {
		throw new Error(
			'Cannot apply diffs to VaultValues of bad mediaType: ' + this.mediaType
		);
	}
...
function detectEncoding(data, encoding) {
	...
	if (encoding && typeof encoding !== 'string') {
		throw new Error(
			'Encoding ' + JSON.stringify(encoding) + ' is not a string.'
		);
	}
...
function detectMediaType(data, mediaType, encoding) {
	...
	if (mediaType && typeof mediaType !== 'string') {
		throw new Error(
			'MediaType ' + JSON.stringify(mediaType) + ' is not a string.'
		);
	}
...
// HACK
// 引数に初期値を設定
VaultValue.prototype.setDataFromVault = function (
	mediaType,
	data,
	encoding = 'live'
) {
...
VaultValue.prototype.setData = function (mediaType, data, encoding = 'live') {
	...
	if (!encoding) {
		throw new Error(
			'Cannot set data for topic "' + this.topic + '" when encoding is unknown'
		);
	}
	...
	if (!mediaType) {
		throw new Error(
			'Cannot set data for topic "' +
				this.topic +
				'" when mediaType is unknown (encoding ' +
				'is "' +
				encoding +
				'", but media type can only be detected when encoding is "live".'
		);
	}
	...
	var forbidReinit =
		encoding === 'live' && this.encodings.live && data === this.encodings.live;
...
VaultValue.prototype.setMediaType = function (toMediaTypes) {
	...
	// HACK
	console.log('[VaultValue] setMediaType');
	console.log('current MediaType:', this.mediaType);
	console.log('convert MediaType:', toMediaTypes);
	...
	for (var i = 0; i < toMediaTypes.length; i++) {
		...
		console.dir(currentMediaTypeApi);
	...
	throw new Error(
		'Unable to convert from mediaType ' +
			this.mediaType +
			' ' +
			'to any of: ' +
			toMediaTypes.join(', ')
