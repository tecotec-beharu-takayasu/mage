...
const couchbase = requirePeer('couchbase');
...
CouchbaseVault.prototype.setup = async function (cfg, cb) {
	...
	if (this.flagStyle !== 'default' && this.flagStyle !== 'node-memcached') {
		var err = new Error(
			'Unknown flagStyle: "' +
				this.flagStyle +
				'" (available: "default", "node-memcached")'
		);
	...
	// 	{
	// 		username: cfg.create.adminUsername,
	// 		password: cfg.create.adminPassword,
	// 	}
	// );
	let host = cfg.options.hosts[0].replace(/\:[0-9]{4}/, '');
	host = `couchbase://${host}`;

	this.cluster = await couchbase.connect(
		host, // 'couchbase://192.168.0.5',
		{
			username: cfg.create.adminUsername,
			password: cfg.create.adminPassword,
		}
...
CouchbaseVault.prototype.open = function (cb) {
	...
	// https://docs.couchbase.com/sdk-api/couchbase-node-client/classes/Cluster.html#bucket

	// this.client = this.cluster.bucket(cfg.options.bucket);
	this.client = this.cluster.bucket(cfg.options.bucket).defaultCollection();


	// HACK
	// 最新版のBucket APIにonがないため、スキップ
	...
	// 	logger.emergency(
	// 		'Couchbase server error for bucket ',
	// 		cfg.options.bucket,
	// 		':',
	// 		getCouchbaseError(error)
	// 	);
	// });
	...
	// HACK
	// 最新版のBucket APIにonがないため、スキップ
	// https://docs.couchbase.com/sdk-api/couchbase-php-client-2.6.0/classes/Couchbase.Bucket.html#method_setTranscoder
	// Create pass-through transcoder functions
	// this.client.setTranscoder(
	// 	couchbaseHelpers.encoder,
	// 	couchbaseHelpers.decoder
	// );
...
CouchbaseVault.prototype.get = async function (key, options, cb) {
	if (!this.client) {
	...
	this.logger.debug('get:', key, 'options:', options);
	...
	// const result = await this.client.scope(key).collection(options.hashkey);
	// const result = await this.client
	// .scope("apRankingHistoryList")
	// .collection("rankingId")
	// .get("1", options);

	const result = await this.client
		// .defaultScope()
		// .defaultCollection()
		.get(key, wash(options));
...
CouchbaseVault.prototype.set = async function (key, data, options, cb) {
	if (!this.client) {
	...
	this.logger.debug('set:', key, 'options:', options);
	...
	// const result = await this.client.collection(key).upsert(options.hashkey, data);
	const result = await this.client
		// .defaultScope()
		// .defaultCollection()
		.upsert(key, data, wash(options), (error) => {
			cb(getCouchbaseError(error));
		});

	this.logger.debug('[CouchbaseVault] set result:');
	console.dir(result);
	cb(result);
	// cb(getCouchbaseError(error));
};
...
CouchbaseVault.prototype.createDatabase = function (cb) {
	...
	var bucketOptions = mage.core.config.get(
		['archivist', 'vaults', vaultName, 'config', 'options'],
		false
	);
	var createBucket = mage.core.config.get(
		['archivist', 'vaults', vaultName, 'config', 'create'],
		false
	);
	if (!createBucket) {
		if (mage.isDevelopmentMode()) {
			logger.warning(
				`No create configuration found for vault ${vaultName}, please refer to the vault's ` +
	...
	if (!mage.isDevelopmentMode()) {
		logger.warning(
			`Create configuration found for ${vaultName} while running in production, this can and will ` +
	...
	// 	createBucket.adminUsername,
	// 	createBucket.adminPassword
	// );

	const clusterManager = cluster.buckets();

	// clusterManager.listBuckets(function (error, buckets) {
	clusterManager.getAllBuckets((error, buckets) => {
		if (error) {
			return cb(getCouchbaseError(error));
		}
		...
		logger.debug(bucketOptions);

		couchbaseHelpers.createBucket(
			logger,
			cluster,
			bucketOptions,
			createBucket,
...
CouchbaseVault.prototype.dropDatabase = function (cb) {
	...
	var clusterManager = this.cluster.manager(
		createBucket.adminUsername,
		createBucket.adminPassword
	);
	clusterManager.listBuckets(function (error, buckets) {
