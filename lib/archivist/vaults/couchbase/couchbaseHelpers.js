...
function waitForSet(logger, cluster, bucketOptions, cb) {
	...
	async.whilst(
		...
		async function (callback) {
			...
			const bucket = cluster.bucket(bucketOptions.bucket).defaultCollection();

			await bucket.upsert(
				'mage/create/test_key',
				{ mageTestKey: 394649 },
				{},
				function (error) {
					// HACK
...
function waitForGet(logger, cluster, bucketOptions, cb) {
	...
	async.whilst(
		...
		async function (callback) {
			...
			// 	bucketOptions.password,
			// 	function (error) {
			// 		if (error) {
			// 			return handleBucketOpenError(error, callback);
			// 		}

			await bucket.get('mage/create/test_key', {}, function (error) {
				// HACK
				// skip error handling
				if (error) {
					// return handleBucketMutationError(error, callback);
				...
				async.whilst(
					...
					async function (callback) {
						...
						await bucket.query(n1qlQuery, function (error) {
							...
							var query =
								'SELECT RAW keyspace_id FROM system:indexes WHERE is_primary = true';
							var n1qlQuery = couchbase.N1qlQuery.fromString(query);
							n1qlQuery.consistency(
								couchbase.N1qlQuery.Consistency.REQUEST_PLUS
							);
			...
			function (callback) {
				...
				async.whilst(
					...
					function (callback) {
						...
						var query =
							'SELECT mageTestKey FROM `' +
							bucketOptions.bucket +
							'` WHERE mageTestKey=394649';
						var n1qlQuery = couchbase.N1qlQuery.fromString(query);
						n1qlQuery.consistency(couchbase.N1qlQuery.Consistency.REQUEST_PLUS);
						...
						bucket.query(n1qlQuery, function (error, results) {
							if (error) {
								...
								keepWaiting =
									keepWaiting ||
									/^An unknown error occured/.test(error.message);
								keepWaiting =
									keepWaiting ||
									/^An unknown N1QL error occured/.test(error.message);
	...
	if (typeof value === 'string' || value instanceof String) {
		...
		if (value.value && value.flags) {
			if (typeof value.value === 'string' || value.value instanceof String) {
				...
				transcoderDoc = {
					value: new Buffer(JSON.stringify(value.value)),
					flags: value.flags,
				};
			}
		} else {
