...
function is_json(data) {
	try {
		...
		// return "content" in JSON.parse(JSON.stringify(data));
	} catch (error) {
		return false;
	}
	return true;
...
Archivist.prototype.get = function (topic, index, options, cb) {
	...
	this.getValue(topic, index, options, function (error, value) {
		...
		try {
			// var d = JSON.stringify(value.data);
		// logger.debug('VALUE_DATA', d);
		// logger.debug('VALUE_DATA', is_json(d));
		// 	logger.debug('VALUE_DATA',  value.data);
		// logger.debug('VALUE_DATA_JSON',  JSON.stringify(value.data));
		// logger.debug('VALUE_DATA_JSON_CONTENT', "content" in JSON.parse(JSON.stringify(value.data)));
		} catch (error) {
			logger.error(error)
		}

		if (value.data !== undefined && is_json(value.data))
		{
		// if (value.data != undefined && "content" in JSON.parse(value.data.to_s)) {
			// data =
			var content = JSON.parse(JSON.stringify(value.data));
			...
			// logger.debug("CONTENT00",content)
			// logger.debug("CONTENT01",typeof content)
